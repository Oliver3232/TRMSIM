...
public final class NodeInspectorHelper {
    private NodeInspectorHelper() {
    }

    public static Vector<String> buildNeighborIds(Sensor sensor) {
    ...
    public static String buildNodeTitle(Sensor sensor) {
        return "Node " + sensor.id() + (sensor.isActive() ? "  ACTIVE" : "  SLEEP");
    }

    public static String buildNodeDetailsText(Sensor sensor, Network currentNetwork) {
    ...
    private static NodeMetric findLatestNodeMetric(int sensorId) {
        List<Outcome> results = es.ants.felixgm.trmsim_wsn.gui.export.SimulationResultRepository.getInstance().getResults();
