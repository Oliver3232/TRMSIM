...
public class PathLengthPanel extends OutcomesPanel {
    ...
    protected Color currentValueColor = new Color(66, 133, 244);
    /** Color used to plot the average accuracy of all the outcomes */
    protected Color averageValueColor = new Color(255, 138, 101);
    ...
    protected void drawAxes(Graphics graphics) {
        ...
        g2.drawString("Path Length Over Recent Simulations", chart.left, chart.top - 18);
        g2.setFont(new Font("SansSerif", Font.PLAIN, 12));
        g2.drawString("Y: Path length [hops]", chart.left, chart.top - 4);
        ...
        for (int tick = 0; tick <= 5; tick++) {
            ...
            g2.setColor(labelColor);
            double tickValue = (maxPathLength * tick) / 5.0;
            g2.drawString(String.format("%.2f", tickValue), Math.max(2, chart.left - 42), y + 4);
            g2.setColor(gridColor);
        }
    ...
    protected void plotOutcomes(Collection<Outcome> outcomes, Graphics graphics) {
        ...
        for (Outcome outcome : outcomes) {
            if (maxPathLength < ((BasicOutcome) outcome).get_avgPathLength()) {
                maxPathLength = Math.ceil(((BasicOutcome) outcome).get_avgPathLength());
            }
        }
        ...
        for (Outcome outcome : outcomes) {
            count++;
            double current = ((BasicOutcome) outcome).get_avgPathLength();
            cumulative += current;
            if ((count - 1) >= start) {
                currentSeries.add(bound01(current / maxPathLength));
                averageSeries.add(bound01((cumulative / count) / maxPathLength));
            }
        }
        ...
        if (!currentSeries.isEmpty()) {
            double current = currentSeries.get(currentSeries.size() - 1) * maxPathLength;
            double avg = averageSeries.get(averageSeries.size() - 1) * maxPathLength;
            g2.setColor(labelColor);
            g2.setFont(new Font("SansSerif", Font.BOLD, 12));
            g2.drawString(String.format("Current: %.2f hops", current), chart.left, chart.bottom + 46);
            g2.drawString(String.format("Running avg: %.2f hops", avg), chart.left + 190, chart.bottom + 46);
        }
    }
    ...
    private void drawSmoothedSeries(Graphics2D g2, List<Double> values, ChartArea chart, Color color, float strokeWidth, String seriesLabel) {
        ...
        if (values.size() == 1) {
            ...
            hoverPoints.add(new HoverPoint(chart.left, (int)Math.round(y),
                    String.format("%s: %.2f hops", seriesLabel, values.get(0) * maxPathLength)));
            return;
        }
        ...
        path.quadTo(xs[xs.length - 2], ys[ys.length - 2], xs[xs.length - 1], ys[xs.length - 1]);
        ...
        for (int i = 0; i < xs.length; i++) {
            ...
            hoverPoints.add(new HoverPoint((int)Math.round(xs[i]), (int)Math.round(ys[i]),
                    String.format("%s #%d: %.2f hops", seriesLabel, i + 1, values.get(i) * maxPathLength)));
        }
    }
    ...
    private double mapY(double normalized, ChartArea chart) {
        return chart.bottom - (chart.height * bound01(normalized));
    }
    ...
    private ChartArea buildChartArea() {
        ...
        int left = Math.max(56, (int) (width * 0.11));
