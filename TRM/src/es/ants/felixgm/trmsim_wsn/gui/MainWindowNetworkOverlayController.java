...
final class MainWindowNetworkOverlayController {
    interface Host {
    ...
    static JPanel createInspectorShellPanel() {
    ...
    static void ensureNetworkOverlayPane(Host host) {
    ...
    static void attachNetworkPanelToOverlay(Host host, NetworkPanel panel) {
        if (panel == null) {
            return;
        }
    ...
    static void layoutNetworkOverlayComponents(Host host) {
        JLayeredPane overlayPane = host.getNetworkOverlayPane();
        if (overlayPane == null) {
            return;
        }
